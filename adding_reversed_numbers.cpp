 ...
 * ancient plays are tragedies. Therefore the dramatic advisor of ACM has decided to transfigure
 * some tragedies into comedies. Obviously, this work is very hard because the basic sense of the
 * play must be kept intact, although all the things change to their opposites. For example the
 ...
 * first digit becomes last and vice versa. For example, if the main hero had 1245 strawberries in
 * the tragedy, he has 5421 of them now. Note that all the leading zeros are omitted. That means if
 * the number ends with a zero, the zero is lost by reversing (e.g. 1200 gives 21). Also note that
 ...
 * their reversed sum. Of course, the result is not unique because any particular number is a
 * reversed form of several numbers (e.g. 21 could be 12, 120 or 1200 before reversing). Thus we
 * must assume that no zeros were lost by reversing (e.g. assume that the original number was 12).
 *
 * Input
 *
 * The input consists of N cases (equal to about 10000). The first line of the input contains only
 * positive integer N. Then follow the cases. Each case consists of exactly one line with two
